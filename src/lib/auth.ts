...
export const { handlers, auth, signIn, signOut } = NextAuth({
            ...
            authorize: async (credentials) => {
                try {
                    ...
                    if (response.data.status === 200 && response.data.user.token) {
                        ...
                        // Restriction: Only Dev users allowed in this panel
                        if (response.data.user.role !== 'dev') {
                            console.log("AUTH_DEBUG: Access denied - role mismatch (expected dev)");
                            return null;
                        }
    ...
    cookies: {
        sessionToken: {
            name: `next-auth.session-token.dev`,
            options: {
                httpOnly: true,
                sameSite: "lax",
                path: "/",
